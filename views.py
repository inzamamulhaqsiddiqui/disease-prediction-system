...
def prediction(req):
    if req.method == 'POST':                      # prediction  posting
        ...
        # now will create a dataframe
        input_df = pd.DataFrame([user_input], columns=symptoms)
        result = model.predict(input_df)[0]
        res = label_encoder.inverse_transform([result])[0]
        
        # now weare going to write the code to save
        # OSM ( Object Relationship Mapping) 
        his = History(fever = fever , headache = headache, nausea = nausea, vomiting = vomiting , fatigue = fatigue, joint_pain = joint_pain, skin_rash = skin_rash, cough = cough, weight_loss = weight_loss, yellow_eyes = yellow_eyes, res = res)       # ( model_field = variable value )
        his.save()   # the data will be saved 
